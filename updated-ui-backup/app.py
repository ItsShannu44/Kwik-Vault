...
class User(UserMixin):
    def __init__(self, id, username, password, profile_picture):
        self.id = id
...
def load_user(user_id):
    conn = sqlite3.connect('chat.db')
    cursor = conn.cursor()
    cursor.execute('SELECT id, username, password, profile_picture FROM users WHERE id = ?', (user_id,))
...
def register():
    ...
    profile_picture = request.files.get('profile_picture')  # Get file from request
    if profile_picture:
        profile_picture_data = profile_picture.read()  # Read binary data
    else:
        profile_picture_data = None  # Set to None if no picture is uploaded
...
#hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh   Profile Pic
@app.route('/get_profile_picture/<int:user_id>', methods=['GET'])
def get_profile_picture(user_id):
    conn = sqlite3.connect('chat.db')
    cursor = conn.cursor()
    
    cursor.execute("SELECT profile_picture, username FROM users WHERE id = ?", (user_id,))
    ...
    if data and data[0]:
        ...
        username = data[1]
        return jsonify({
            "profile_picture": f"data:image/jpeg;base64,{encoded_image}",
            "username": username
        })
...
def handle_send_message(data):
    ...
    emitted_data = {**data, 'sender_username': current_user.username}


    # Emit to both sender and recipient
    emit('receive_message', {**data, 'sender_username': current_user.username})
    emit('receive_message', {**data, 'sender_username': current_user.username})
