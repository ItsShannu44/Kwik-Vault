...
def register():
    ...
    if not password_pattern.match(password):
        flash("Password must be at least 6 characters long and include an uppercase letter, "
              "a lowercase letter, a number, and a special character.", "danger")
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('INSERT INTO users (username, password, profile_picture, email) VALUES (?, ?, ?, ?)', (username, password, profile_picture_data, email))
        conn.commit()
